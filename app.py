...
class DeepSeekLLM(LLM):
    ...
    def __init__(
        ...
        temperature: float = 0.6,
        max_tokens: int = 512,
        callback_manager: Optional[CallbackManager] = None,  # Fixed syntax error here
    ):
...
def show_admin_controls():
    ...
    uploaded_files = st.sidebar.file_uploader(
        "Upload Documents", 
        type=["pdf", "txt", "png", "jpg", "jpeg", "xlsx", "docx"],  # Added more file types
    ...
    if uploaded_files:
        if st.sidebar.button("Process Documents", key="process_docs_button"):
            with st.sidebar.expander("Processing Details", expanded=True):
            ...
            for file in stqdm(uploaded_files):
                try:
                    ...
                    if file.type == 'application/pdf':
                        ...
                        st.sidebar.info("Using OCR for image processing...")
                    ...
                    if result['success']:
                        ...
                        try:
                            ...
                            if result['metadata'].get('ocr_used'):
                                st.sidebar.info("OCR was used to extract text")
                        except Exception as e:
